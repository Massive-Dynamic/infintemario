...
Mario.LevelState.prototype.Draw = function(context) {
	...
	myScoreFixed = window.myScore/10;
	myScoreFixed = myScoreFixed.toFixed(0) * 10;
    this.DrawStringShadow(context, "MARIO " + Mario.MarioCharacter.Lives, 0, 0);
    this.DrawStringShadow(context, myScoreFixed.toString(), 0, 1);
    ...
    if (Mario.MarioCharacter.WinTime > 0) {
        ...
        if (t > 900) {
			...
			//TODO: figure out bonus dependent on flag height (+2500 standard for now) + 50 * time left with 100 minimum
			var bonusTimeScore = Math.ceil(this.TimeLeft) > 1 ? Math.ceil(this.TimeLeft * 50) : 100;
			window.myScore += (2500 + bonusTimeScore);
        }
